<form method="POST" action="{{ route('login') }}">
  ...
  <div class="relative mb-3 w-full">
    <input id="email" name="email" type="email" value="{{ old('email') }}" style="display: hidden;"
      wire:model='email' hidden />
  </div>
  <div class="inputbox relative mb-3 w-full">
    <input type="text" value="{{ old('email') }}" wire:change='chech_for_email' required autocomplete="email"
      autofocus wire:model.live='username_email_or_phone' />
